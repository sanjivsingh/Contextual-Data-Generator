...
public class MACAddress {
	 ...
	 * 
	 * @param theMac
	 *            - the mac string "00:0d:dd:e1:12:4f" The max should include
	 *            all bytes including the padding zeros
	 */
	public MACAddress(String theMac) {
		...
		for (int i = NUM_OF_FIELDS - fields.length; i < NUM_OF_FIELDS; i++) {
			myBytes[i] = Integer.decode(
					"0x" + fields[i - (NUM_OF_FIELDS - fields.length)])
					.byteValue();
		}
	}
	 ...
	 * 
	 * @param theMacAsInt
	 *            - the mac value. ( example "00:0d:00:00:01:4f" == 335)
	 */
	 ...
	 */
	public String toReadbleString(char c) {
		...
		for (int i = 0; i < myBytes.length; i++) {
			...
			char second = (char) ((second1 < 10) ? '0' + second1
					: 'A' + second1 - 10);
			char first = (char) ((first1 < 10) ? '0' + first1
					: 'A' + first1 - 10);
